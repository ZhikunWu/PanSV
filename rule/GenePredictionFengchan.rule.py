...
#         R1 = IN_PATH + '/clean/CPG20.RNA.R1.fq.gz',
#         R2 = IN_PATH + '/clean/CPG20.RNA.R2.fq.gz',
#     output:
...
rule TrinityGuideAssembly:
    input:
        bam = IN_PATH + "/GenePrediction/RNA/hisat/Fengchan6/ngs_RNA_hisat.bam",
    output:
        fa = IN_PATH + "/GenePrediction/RNA/trinity/trinity_out_dir/Trinity-GG.fasta",
    params:
        outDir = IN_PATH + "/GenePrediction/RNA/trinity",
    threads:
        THREADS
    log:
        IN_PATH + "/log/TrinityGuideAssembly.log",
    run:
        ...
        shell("cd {params.outDir} && Trinity --genome_guided_bam {input.bam} --genome_guided_max_intron 10000 --max_memory 100G --CPU {threads} > {log} 2>&1")
...
rule cufflinks:
    input:
        bam = IN_PATH + "/GenePrediction/RNA/hisat/Fengchan6/ngs_RNA_hisat.bam",
    output:
        gtf = IN_PATH + "/GenePrediction/RNA/cufflinks/transcripts.gtf",
    params:
        outDir = IN_PATH + "/GenePrediction/RNA/cufflinks",
    threads:
        THREADS
    log:
        IN_PATH + "/log/trim/cufflinks.log",
    run:
...
rule scallop:
    input:
        bam = IN_PATH + "/GenePrediction/RNA/hisat/Fengchan6/ngs_RNA_hisat.bam",
    output:
        gtf = IN_PATH + "/GenePrediction/RNA/scallop/transcripts.gtf",
    threads:
        THREADS
    log:
        IN_PATH + "/log/trim/scallop.log",
    run:
...
rule cuffmerge:
    input:
        stringtie = IN_PATH + "/GenePrediction/RNA/Stringtie/Fengchan6/Fengchan6_RNA_stringtie.gtf",
        cufflinks = IN_PATH + "/GenePrediction/RNA/cufflinks/transcripts.gtf",
        scallop = IN_PATH + "/GenePrediction/RNA/scallop/transcripts.gtf",
        assembly = IN_PATH + "/Assembly/Scaffold/Fengchan6.scaffold.fasta",
    output:
        transList = IN_PATH + "/GenePrediction/RNA/assembly_merge_list.txt",
        gtf = IN_PATH + "/GenePrediction/RNA/cuffmerge/merged.gtf",
    params:
        outDir = IN_PATH + "/GenePrediction/RNA/cuffmerge",
    log:
        IN_PATH + "/log/cuffmerge.log",
    run:
